...
/*   Updated: 2021/04/06 14:31:36 by lmarzano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_keys(t_vars *vars) //32 lines
{
	if (vars->keyUp == 1) //w
	{
		if (ft_iscinstr(vars->map[(int)(vars->posX + vars->dirX * 0.4)] \
		 [(int)(vars->posY)], "0NSWE", 5))
			vars->posX = vars->posX + vars->dirX * vars->moveSpeed;
		if (ft_iscinstr(vars->map[(int)(vars->posX)] \
		 [(int)(vars->posY + vars->dirY * 0.4)], "0NSWE", 5))
...
int 	ft_keys2(t_vars *vars)
{
	...
	{
		if (ft_iscinstr(vars->map[(int)(vars->posX \
		 + vars->planeX * 0.4)][(int)(vars->posY)], "0NSWE", 5))
			vars->posX = vars->posX + vars->planeX * vars->moveSpeed;
		if (ft_iscinstr(vars->map[(int)(vars->posX)][(int)(vars->posY \
		 + vars->planeY * 0.4)], "0NSWE", 5))
			vars->posY = vars->posY + vars->planeY * vars->moveSpeed;
	}
	if (vars->keyRight == 1) //a
	{
		if (ft_iscinstr(vars->map[(int)(vars->posX - vars->planeX * 0.4)] \
		 [(int)(vars->posY)], "0NSWE", 5))
			vars->posX -= vars->planeX * vars->moveSpeed;
		if (ft_iscinstr(vars->map[(int)(vars->posX)] \
		 [(int)(vars->posY - vars->planeY * 0.4)], "0NSWE", 5))
